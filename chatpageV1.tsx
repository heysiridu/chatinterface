...
const ChatPage = ({ chatId, sessionObj, share }: ChatPageProps) => {
    ...
    useEffect(() => {
        ...
        const fetchSetMessages = async (chatId: string) => {
            setIsLoading(true);
            if (sendBtn.current) {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            const isAtBottom = Math.abs(chatElem.scrollTop + chatElem.clientHeight - chatElem.scrollHeight) < 10;
            setUserScroll(!isAtBottom);
        };
    ...
    // Popup click outside handler
    useEffect(() => {
        const clkOut = () => setShowPopup(false);
        ...
        return () => document.removeEventListener('click', clkOut);
    }, [showPopup]);

    const changeSendBtnStatus = (accept: boolean) => {
        if (sendBtn.current) {
            sendBtn.current.disabled = !accept;
            if (accept) {
                setIsLoading(false);
                sendBtn.current.classList.remove("animate-pulse", "animate-spin");
                sendBtn.current.innerHTML = "arrow_upward";
                sendBtn.current.style.backgroundColor = "#ffffff";
            } else {
                setIsLoading(true);
                sendBtn.current.classList.add("animate-pulse", "animate-spin");
    ...
    const msgSend = async (nIdPrompt?: string) => {
        ...
        if (msg !== "") {
            ...
            if (!stream) {
                console.error("Error: stream not found");
                return;
            }
    ...
    const renderToolButton = (toolName: string, icon: string) => (
        <button
            className='gap-2 min-h-10 min-w-10 flex items-center justify-center border-[#d3d3d3] rounded-full hover:bg-[#50505039] cursor-pointer transition-colors shadow-lg'
            onClick={() => {
                if (chatType === toolName) {
                    setChatType("");
                    return;
                }
                setChatType(toolName);
            }}
            style={{
                border: chatType === toolName ? "2px solid #007bff" : "2px solid transparent",
                color: chatType === toolName ? "#007bff" : "#d3d3d3",
                paddingLeft: chatType === toolName ? "16px" : "0",
                paddingRight: chatType === toolName ? "16px" : "0",
            }}
        >
            <span className='material-icons' style={{ fontSize: 20 }}>{icon}</span>
            {chatType === toolName && (
                <span className='text-[14px] font-medium'>{toolName}</span>
            )}
        </button>
    );
        ...
        <div className='w-full rounded-3xl bg-[#303030] p-4'>
            ...
            <div className='flex flex-row justify-between items-center mt-4'>
                <div className='flex flex-row gap-2'>
                    {renderToolButton("Build", "build")}
                    {renderToolButton("Edit", "edit")}
                    {renderToolButton("Analyze", "bar_chart")}
                </div>
                <button
                    onClick={() => msgSend()}
                    ref={sendBtn}
                    className='material-icons bg-white text-black rounded-full p-1.5 hover:bg-gray-100 transition-colors duration-300 cursor-pointer'
                >
                    arrow_upward
                </button>
            </div>
        ...
        <>
            ...
            {shareOpen && visib !== "" && (
                <ChatShare
                    ...
                    chatID={chatId}
                    visibility={visib}
                    changeVisibility={(newVis) => setVisib(newVis)}
                />
            )}
            ...
            <div className='flex flex-row h-screen w-screen text-white overflow-hidden font-[Poppins] bg-[#212121]'>
                {share && (
                    <div
                        ref={sideMenu}
                        className='w-[270px] bg-black overflow-y-auto deskver:static absolute left-0 h-screen z-100 deskver:block hidden'
                        style={{ scrollbarColor: "#444 #000" }}
                    >
                        <div className='flex sticky top-0 flex-col bg-[inherit]'>
                            <div className='flex flex-row p-3 px-4 justify-between w-full items-center'>
                                ...
                                <button
                                    ...
                                    onClick={toggleSideMenu}
                                >
                                    <span className="material-symbols-outlined text-gray-200">dual_screen</span>
                                </button>
                                <button
                            ...
                            <div
                                ...
                                style={{ alignItems: sideMenuOpen ? "initial" : "center" }}
                            >
                                <button
                                    ...
                                    style={{ width: sideMenuOpen ? "100%" : "fit-content" }}
                                    onClick={() => router.push("/chat")}
                                >
                                    <span
                                        className="material-symbols-outlined"
                                        style={{ fontSize: 19, fontWeight: 300, color: "#ffffff" }}
                                    >
                                        add_comment
                                    </span>
                                    {sideMenuOpen && (
                        ...
                        {sideMenuOpen && (
                            <div className='text-white flex flex-col px-2 py-3 gap-1'>
                                <span className='text-[#999] text-[14px] px-3'>History</span>
                                <div>
                ...
                >
                                        ...
                                        <div className="relative inline-block">
                                            ...
                                            {showPopup && (
                                                <div className="absolute top-full left-0 bg-[#353535] rounded-xl py-2 px-1.5 z-10 w-fit gap-1.5 flex flex-col">
                                                    <button
                                                        className="w-full flex flex-row items-center cursor-pointer rounded-lg text-white hover:bg-white/10 text-sm gap-2 px-3 py-1.75"
                                                        onClick={() => setShareOpen(true)}
                                                    >
                                                        <span
                                                            className="material-symbols-outlined"
                                                    ...
                                                    <button
                                                        className="w-full flex flex-row items-center cursor-pointer rounded-lg text-red-400 hover:bg-red-500/10 text-sm gap-2 px-3 py-1.75"
                                                        onClick={deleteConv}
                                                    >
                                                        <span
                                                            className="material-symbols-outlined"
                    ...
                    <div
                        className='h-full w-full p-4 pb-0 flex flex-col max-w-[700px] mx-auto'
                        style={{
                    ...
                    >
                        {chatDisplay.length === 0 && !chatId ? (
                            <>
                                <div className='deskver:flex hidden flex-col mb-10 items-center justify-center text-4xl text-center'>
                                    ...
                                    <span className='bg-gradient-to-r bg-clip-text text-transparent from-[#b2b2b4] via-[white] to-[#b2b2b4]'>
                            ...
                            <div className="space-y-2 mb-[calc(15px+100px)] flex flex-col gap-2" ref={chatLogs}>
                                {chatDisplay.map((msg, index) => (
                                    <div
                                        ...
                                        className='w-fit'
                                        style={{
                                            maxWidth: msg.sender === "user" ? "500px" : "initial",
                                    ...
                                    >
                                        <div className='[&>p:nth-last-of-type(1)]:inline'>
                        ...
                        {share && (
                            <>
                                <div
                                    className='w-full deskver:block hidden'
                                    style={{
                                ...
                                <div
                                    className='w-full deskver:hidden block'
                                    style={{
                                        position: "sticky",
                                        bottom: "15px",
                                        marginTop: "auto",
